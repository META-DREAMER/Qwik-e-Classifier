...
To go with our Linux image provided by thinkoco, we will also use their board support package (BSP).
...
I am most familiar with Rust, so the example will be written in it, but any 
...
In the FAT32 partition of the SD card their is a file named `socfpga.dtb` which is a "device tree blob".
