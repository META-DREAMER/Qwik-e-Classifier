...
TIP: Run the included `setup_cl_tools.sh` script on the DE10 to do all these steps for you! 
See <<Transferring files to DE10>> for how to copy the script (or other files) over.
...
On most laptops you can also share the laptops wifi to ethernet, to connect that way if on a network that whitelists ethernet connections.
