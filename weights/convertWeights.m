...
list2 = [981, 726, -400, -1353, -10194, -1941, 92, 1767, 0];
...
%%
A = imread('../PipeCNN/project/data/yolo/dog.jpg');
B = imresize(A, [416, 416]);

ImgVector = reshape(B, [], 1);
dog = double(ImgVector) / double(255);
for i = 1:length(dog)
   dog(i) = storedInteger(fi(dog(i),1, 8, 7));
   disp(i);
end

%%

fileID = fopen('../PipeCNN/project/data/yolo/dog.dat','w');
fwrite(fileID, dog, 'int8');
fclose(fileID);
